...
class QuantizedDense(tf.keras.layers.Layer):
    def __init__(self, units, kernel_initializer, bias_initializer, **kwargs):
        ...
        # Create internal dense layer without activation
    ...
    def call(self, inputs):
        ...
        # Quantize activation with per-batch min/max
        min_a = tf.reduce_min(output)
        max_a = tf.reduce_max(output)
        output = tf.quantization.fake_quant_with_min_max_vars(
            output, 
            min=min_a, 
            max=max_a,
            num_bits=8,
...
def main():
    ...
    detnet.save("detnet_qpsk_relu_quantized.keras")
    print("Model saved as detnet_qpsk_relu_quantized.keras")
